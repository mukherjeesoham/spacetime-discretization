...
S_d = \dfrac{1}{2} \sum\limits_{p=0}^{N\times N}\sum\limits_{q=0}^{N\times N} w^{pq}\, \eta_{ab} \, D_{pk}^a u^{k}\, D_{ql}^b u^{l} 
\end{equation}
...
grid. Further, the derivative operator $D^a_{pk}$ corresponds to the
derivative of the vector $u^k$ evaluated at $p$. Concretly, the operator can be expressed as 
\begin{equation}
\eta_{ab} \, D_{pk}^a\, D_{ql}^b = - \rm{I}\otimes D_t^2 +  D_x^2\otimes\rm{I}
\end{equation}
for a (1+1) dimensionsal case, which gives a square matrix of dimension $(N\times N)$.\\
...
\textcolor{red}{linear} equations given by Eq.~(\ref{main}). \\

For imposing boundary conditions, we use the method of Lagrange multipliers, since this seems the 
most general and intiutive way to do this.
