...
# choose points where the Neumann BCs needs to
# be enforced.
loc_BN[0] =  1
BN = np.dot(np.diag(np.ravel(loc_BN)), np.kron(I,D0))	
...
BN = np.lib.pad(BN, (0, len(BD[BD==1])), 'constant', constant_values=(0))
...
uu[:, 0]  = 0
uu[:, -1] = 0
uu[0]     = -np.exp(-(x)**2.0/2.0*25.0)	# gaussian

u = np.ravel(uu)
u = u[np.where(np.ravel(loc_BD)==1)[0]]

# set the Dirichlet boundary values
duu    = np.zeros((N+1, N+1))
duu[0] = 0.004*x*np.exp(-(x)**2.0/2.0*25.0)

du = np.ravel(duu)
du = du[np.where(np.ravel(loc_BN)==1)[0]]
...
if(1):	# find Eigenvalues (w) and Eigen vectors (v)
	...
	plt.savefig("../output/scalar_wave_eigen_values.png")
	plt.close()
...
if(1):	#plot the solution
	...
	plt.savefig("../output/scalar_wave_solution.png")
	plt.close()
	...
	for index, i in enumerate(Z):
		...
		plt.xlim(-1, 1)
		plt.ylim(-1, 1)
		plt.savefig("../output/motion/time%r.png"%(index+100))
		plt.close()